...
fn applyFog(color: vec3f, dist: f32, rd: vec3f) -> vec3f {
  ...
  var fogColor = skyColor(rd) * scene.fogSkyScale + scene.ambientColor;
...
fn shade(p: vec3f, normal: vec3f, materialId: u32) -> vec3f {
  ...
  let moonShadow = calcSoftShadow(
    moonShadowOrigin,
    moonDir,
    0.02,
    scene.moonShadowMaxDistance,
    scene.moonShadowK,
  );
  ...
  color += scene.ambientColor * scene.ambientStrength;
  ...
  for (var i = 0; i < 10; i++) {
    ...
    let attenuation = max(light.attenuation, 0.0001);
    ...
    let shadowK = max(light.shadowK, 0.1);
    let shadowMaxDistance = min(lightDist, max(light.shadowMaxDistance, 0.05));
    let houseShadow = calcSoftShadow(houseShadowOrigin, lightDir, 0.02, shadowMaxDistance, shadowK);

    color += light.color * lightDiffuse * lightAtten * houseShadow * scene.pointLightDiffuseScale;
  }
